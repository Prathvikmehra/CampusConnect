...
export const RolesManager: React.FC<RolesManagerProps> = ({ clubId }) => {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      const formattedRoles: ClubRole[] = (data || []).map((role: any) => ({
        ...
        permissions: (role.club_role_permissions || []).map(
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      setError(
        err.message.includes("duplicate") ? "A role with this name already exists." : err.message,
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDeleteRole = async (roleId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this role? Members assigned to it will lose their permissions.",
      )
    )
      return;

    try {
      const { error: deleteError } = await supabase.from("club_roles").delete().eq("id", roleId);
      ...
      setError(
        err.message.includes("Cannot delete system roles")
          ? "Cannot delete the President role."
          : err.message,
      );
    }
  };
