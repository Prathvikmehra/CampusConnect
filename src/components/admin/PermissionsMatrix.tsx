...
export const PermissionsMatrix: React.FC<PermissionsMatrixProps> = ({
  ...
  const [selectedPerms, setSelectedPerms] = useState<Set<AppPermission>>(
  ...
  const togglePermission = (perm: AppPermission) => {
    ...
    if (isSystemRole && perm === "can_manage_roles" && selectedPerms.has(perm)) {
      alert(
        'Cannot remove "Manage Roles" from the President role. This would lock you out of the admin panel.',
      );
      return;
    }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">
            Permissions for:{" "}
            <span className="text-indigo-600 dark:text-indigo-400">{roleName}</span>
          </h3>
      ...
      {PERMISSION_CATEGORIES.map((category) => {
        const categoryPerms = (Object.keys(PERMISSION_METADATA) as AppPermission[]).filter(
